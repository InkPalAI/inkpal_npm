/**
 * Progress tracking tests — Phase 2 Block 2.1.
 */
...
afterEach(() => { try { rmSync(PROJ, { recursive: true, force: true }); } catch { /* skip */ } });
...
describe('Progress tracking — get/set/list/append', () => {
  it('list on empty project returns 0 features', async () => {
    ...
    const data = r.data as { feature_count: number; summary: unknown[] };
  ...
  it('get on a known feature returns its state', async () => {
    ...
    expect(r.success).toBe(true);
    expect((r.data as { status: string }).status).toBe('in_progress');
  });
  ...
  it('append_done auto-promotes status: not_started → in_progress', async () => {
    ...
    expect(r.success).toBe(true);
    const data = r.data as { status: string; completed: string[]; pending: string[] };
  ...
  it('append_done auto-flips to done when pending empties', async () => {
    ...
    const data = r.data as { status: string; pending: string[] };
  ...
  it('append_pending adds to pending list (no dup)', async () => {
    ...
    const data = r.data as { pending: string[] };
    expect(data.pending).toEqual(['dark mode toggle']);  // no dup
  });
  ...
  it('note appends with timestamp', async () => {
    ...
    const data = r.data as { notes: string[] };
    expect(data.notes.length).toBe(1);
    expect(data.notes[0]).toContain('API team blocked on schema');
    expect(data.notes[0]).toMatch(/^\d{4}-\d{2}-\d{2}T/);  // ISO timestamp prefix
  });
  ...
  it('list summary computes pct_complete correctly', async () => {
    ...
    const r = await handleLocal('inkpal_progress', { action: 'list', project_path: PROJ }) as Record<string, unknown>;
    const data = r.data as { summary: Array<{ feature: string; pct_complete: number }> };
    expect(data.summary[0].pct_complete).toBe(50);  // 2/4
  });
});
