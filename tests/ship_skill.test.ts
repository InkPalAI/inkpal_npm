/**
 * /inkpal:ship mechanical acceptance — 5/5 runs through the chain executor.
 *
 * Runs auto_commit on a temp git repo (no real PRs to anywhere), exercises
 * the gate logic + commit envelope. PR creation step skipped (would need
 * gh CLI auth + remote repo).
 */
...
describe.runIf(HAS_LICENSE)('/inkpal:ship mechanical — auto_commit + pre_merge_gate', () => {
  ...
  it('create_pr without gh installed returns gh_cli_not_installed', async () => {
    // We cannot guarantee gh is missing on dev machines, so skip the assertion
    // if it IS installed. The test verifies the error envelope shape.
    ...
    if (r.error === 'gh_cli_not_installed') {
      ...
      // gh is installed — different error path is fine
  ...
  it('pre_merge_gate runs all 4 gates and returns structured breakdown', async () => {
    ...
    const gates = r.gates as Array<{ name: string; passed: boolean }>;
    expect(gates.length).toBe(4);
    expect(gates.map(g => g.name)).toEqual(['flutter_analyze', 'check_safety', 'audit_ui', 'accessibility_audit']);
  }, 30_000);

  // Single full chain mechanical run (we run once, not 5x — chain is deterministic)
  it('/ship chain rails: 5 mechanical runs all complete without crash', async () => {
    for (let i = 0; i < 5; i++) {
      const r = await runChain([
        ...
        // Skip auto_commit + create_pr in mechanical to avoid creating real commits
        // each loop. Real /ship adds these; rails verified above.
