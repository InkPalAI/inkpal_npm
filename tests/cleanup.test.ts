/**
 * Cleanup tests — preserve list, retention, size cap.
 */
...
afterEach(() => {
  try { rmSync(TMP_PROJ, { recursive: true, force: true }); } catch { /* skip */ }
});
...
describe('Cleanup — preserves user value, deletes ephemeral', () => {
  ...
  it('finalizeSession writes summary.json and deletes intermediate', () => {
    ...
    // Empty failures.jsonl gets deleted
