...
async function httpJson(url: string, body: unknown, timeoutMs = 8000): Promise<{
    status: number;
    data?: any;
}> {
...
async function promptAndClaimTrial(cfg: Config): Promise<{
    key: string;
    tier: string;
    auto: boolean;
    email?: string;
...
async function refreshTier(licenseKey: string, cachedTier?: string): Promise<{
    tier: string;
    upgraded: boolean;
    downgraded: boolean;
}> {
...
function patchMainDart(cwd: string): {
    changed: boolean;
    reason?: string;
} {
    const path = join(cwd, 'lib', 'main.dart');
    if (!existsSync(path))
        return { changed: false, reason: 'lib/main.dart missing' };
    let src = readFileSync(path, 'utf8');
    const before = src;
    ...
    if (!hasImport) {
        if (/import\s+['"]package:flutter\/material\.dart['"];?/.test(src)) {
            src = src.replace(/(import\s+['"]package:flutter\/material\.dart['"];?)/, `$1\nimport 'package:inkpal_bridge/inkpal_bridge.dart';`);
        }
        else {
...
async function ensureLicense(): Promise<string | null> {
    ...
    if (key && /^ink_\S+/.test(key)) {
        ...
        if (!claimed) {
            fail('License trial failed', 'no key issued');
            return null;
        }
        key = claimed.key;
    ...
    if (refresh.upgraded) {
        ...
        if (cfg.cached_tier !== refresh.tier)
            saveConfig({ ...loadConfig(), cached_tier: refresh.tier, cached_tier_at: new Date().toISOString() });
    }
    return key!;
}
...
function finalizeProject(cwd: string): void {
    try {
        execSync('flutter pub get', { cwd, encoding: 'utf8', timeout: 90000, stdio: 'pipe' });
        ok('flutter pub get');
    }
    catch (e) {
...
async function doctor(cwd: string, licenseKey: string): Promise<{
    healthy: boolean;
    results: typeof checks;
}> {
    const flutter = (() => { try {
        execSync('flutter --version', { stdio: 'pipe', timeout: 10000 });
        return true;
    }
    catch {
        return false;
...
export async function start(): Promise<void> {
    ...
    for (const host of hosts)
        installMcp(host, licenseKey, cwd);
    finalizeProject(cwd);
    console.log('\n──── Doctor ────');
