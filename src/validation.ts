...
import { handleLocal, getRegistry, isRegistered, LOCAL_TOOLS, type RegistryToolEntry, type RegistryResponse, } from './local.js';
...
function readAllSessionSummaries(): Array<{
    session_id: string;
    ok: boolean;
...
function checkCrossToolConsistency(reg: RegistryResponse | null): CrossToolFinding[] {
    ...
    for (const producer of reg.tools) {
        const nextList = (producer as RegistryToolEntry & {
            next_logical?: string[];
        }).next_logical ?? [];
        for (let i = 0; i < nextList.length; i++) {
            ...
            const produces = (producer as RegistryToolEntry & {
                produces?: string[];
            }).produces ?? [];
            const consumes = (consumer as RegistryToolEntry & {
...
function auditFallback(reg: RegistryResponse | null): FallbackFinding[] {
    ...
    for (const ev of failures as Array<{
        kind?: string;
        tool_name?: string;
...
async function checkDeterminism(reg: RegistryResponse | null): Promise<DeterminismFinding[]> {
    ...
    for (const tool of reg.tools) {
        ...
        if (!ex)
            continue;
        const deps = (tool as RegistryToolEntry & {
            deps?: {
                required?: string[];
            };
        }).deps;
        if (deps?.required?.some(d => d.includes('alive') || d.includes('baseline')))
            continue;
...
async function simulateFailures(): Promise<FailureSimFinding[]> {
    ...
    const hasFallbackLog = (fails as Array<{
        fallback_used?: boolean;
    }>).some(f => f.fallback_used === true);
    findings.push({
...
function buildLatencyBreakdowns(): LatencyBreakdown[] {
    ...
    const byChain = new Map<string, Array<{
        session_id: string;
        total_ms: number;
        steps: Array<{
            tool_name: string;
    ...
    for (const [chain, runs] of byChain.entries()) {
        ...
        if (slowest.pct >= 60)
            target = `${slowest.tool_name} dominates ${slowest.pct}% — consider batching/caching`;
        else if (slowest.pct >= 35)
            target = `${slowest.tool_name} is the heaviest step (${slowest.pct}%)`;
        breakdowns.push({
            chain,
...
async function evaluateOutputQuality(reg: RegistryResponse | null): Promise<OutputQualityFinding[]> {
    ...
    for (const tool of reg.tools) {
        ...
        try {
            ...
            if (!r)
                notes.push('response was null/undefined');
            let clarity = 7;
            if (r?.success === false) {
                if (r?.hint)
                    clarity += 1;
                    ...
                    clarity -= 2;
                    notes.push('error response missing hint');
                }
                if (r?.recovery_tool || r?.next_tool)
                    clarity += 1;
            }
            const fieldCount = r ? Object.keys(r).length : 0;
            const usefulness = fieldCount >= 4 ? 9 : fieldCount >= 2 ? 6 : 3;
            if (fieldCount < 2)
                notes.push(`response has only ${fieldCount} field(s) — likely raw stdout`);
            findings.push({
...
export async function runFullValidation(): Promise<SystemValidationReport> {
    ...
    const overall: SystemValidationReport['summary']['overall_health'] = blockers > 0 ? 'red' : (high > 2 ? 'yellow' : 'green');
    return {
