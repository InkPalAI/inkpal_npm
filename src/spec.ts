    ...
    switch (action) {
        case 'parse': {
            ...
            const pubspec_yaml = existsSync(pubspecPath)
                ? (() => { try {
                    return readFileSync(pubspecPath, 'utf8');
                }
                catch {
                    return undefined;
                } })()
        ...
        case 'get': {
            if (!args.plan_id)
                return { success: false, action, project_path: projectPath, error: 'plan_id_required' };
            const path = join(specsDir, `${args.plan_id}.json`);
            if (!existsSync(path))
                return { success: false, action, project_path: projectPath, error: 'plan_not_found', plan_id: args.plan_id };
