/**
 * Progress tracking — Phase 2 Block 2.1.
 *
 * Per-project state at <project_path>/.inkpal/progress.json. Survives team
 * handoff via git (it's in the project tree). Survives proxy restarts.
 *
 * State shape:
 *   { features: { <featureName>: { status, completed[], pending[], notes[],
 *                                  created_at, updated_at } } }
 *
 * 4-state vocabulary: not_started | in_progress | blocked | done.
 *
 * Two consumption paths:
 *   - Skill chains auto-call set/append on completion (via T-PRG-001 inkpal_progress)
 *   - Users call get/list directly via raw tool call or `/inkpal:status` (future)
 *
 * No new department, no new skill at v1. The single tool inkpal_progress
 * dispatches via {action: 'get' | 'set' | 'list' | 'append_done' | 'append_pending'}.
 *
 * Discipline:
 *   Rule 1: 1 new tool, slots into existing UNDERSTAND department
 *   Rule 4: writes are explicit; no silent state mutation
 *   Cleanup compatibility: progress.json is in PRESERVE_NAMES (user value, NOT ephemeral)
 */
...
function load(projectPath: string): ProgressFile {
  ...
  if (!existsSync(path)) return emptyFile(projectPath);
  try {
    const data = JSON.parse(readFileSync(path, 'utf8')) as ProgressFile;
    // Guard against shape drift
    if (data.version !== '1.0.0' || !data.features) return emptyFile(projectPath);
    return data;
  } catch { return emptyFile(projectPath); }
}
...
// ── Public API (called by inkpal_progress tool handler) ─────────────────
...
export interface ProgressResponse {
  ...
  project_path: string;
  feature?: string;
  data?: FeatureProgress | Record<string, FeatureProgress> | { feature_count: number; summary: Array<{ feature: string; status: FeatureStatus; pct_complete: number }> };
  hint?: string;
  error?: string;
}
...
export function handleProgress(args: ProgressArgs): ProgressResponse {
  ...
  switch (action) {
    ...
    case 'get': {
      if (!args.feature) {
        // No feature → return all features
        return {
          success: true, action, project_path: projectPath,
          data: file.features,
    ...
    case 'set': {
      if (!args.feature) return { success: false, action, project_path: projectPath, error: 'feature_required' };
      const fp = ensureFeature(file, args.feature);
      if (args.status) fp.status = args.status;
      if (args.completed) fp.completed = args.completed;
      if (args.pending) fp.pending = args.pending;
    ...
    case 'append_done': {
      if (!args.feature || !args.done_item) return { success: false, action, project_path: projectPath, error: 'feature_and_done_item_required' };
      const fp = ensureFeature(file, args.feature);
      if (!fp.completed.includes(args.done_item)) fp.completed.push(args.done_item);
      // Auto-remove from pending if present
      fp.pending = fp.pending.filter(p => p !== args.done_item);
      // Auto-status: if all pending cleared and at least one completed, mark done
      if (fp.pending.length === 0 && fp.completed.length > 0) fp.status = 'done';
      else if (fp.completed.length > 0) fp.status = 'in_progress';
      fp.updated_at = new Date().toISOString();
      save(projectPath, file);
      return {
        success: true, action, project_path: projectPath, feature: args.feature, data: fp,
    ...
    case 'append_pending': {
      if (!args.feature || !args.pending_item) return { success: false, action, project_path: projectPath, error: 'feature_and_pending_item_required' };
      const fp = ensureFeature(file, args.feature);
      if (!fp.pending.includes(args.pending_item) && !fp.completed.includes(args.pending_item)) {
        fp.pending.push(args.pending_item);
      }
      if (fp.status === 'not_started' && fp.pending.length > 0) fp.status = 'in_progress';
      fp.updated_at = new Date().toISOString();
      save(projectPath, file);
      return {
        success: true, action, project_path: projectPath, feature: args.feature, data: fp,
    ...
    case 'note': {
      if (!args.feature || !args.note) return { success: false, action, project_path: projectPath, error: 'feature_and_note_required' };
      const fp = ensureFeature(file, args.feature);
