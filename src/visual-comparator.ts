...
export async function compare(baselinePath: string, currentPath: string, options: CompareOptions = {}): Promise<ComparisonResult> {
    ...
    try {
        const pm = await (Function('return import("pixelmatch")')() as Promise<{
            default: typeof pixelmatch;
        }>);
        pixelmatch = pm.default;
        const pngjs = await (Function('return import("pngjs")')() as Promise<{
            PNG: typeof PNG;
        }>);
        PNG = pngjs.PNG;
    }
    ...
    const differingPixels = pixelmatch(baselineData, currentData, diffData, width, height, { threshold, includeAA, alpha });
...
function padImage(data: Buffer | Uint8Array, srcW: number, srcH: number, targetW: number, targetH: number): Uint8Array {
