...
## Wire to Claude Code (the one-line path)
...
That single command:
- Adds `inkpal_bridge` to `pubspec.yaml`
- Patches `lib/main.dart` (AST-safe, idempotent)
- Installs the MCP server into every detected editor (Claude Code, Cursor, Windsurf, Codex, Copilot)
- Validates your license + upgrades your tier
- Runs a 5-second first-success demo (`audit_ui` on your project)

Restart your editor and verify with `/mcp` in Claude Code. Expect:
```
[inkpal] ready — N tools available
[inkpal] License OK — Pro tier active.
```

Once that's done, in Claude Code just ask:

> "Audit my UI for layout issues"
> "Inspect the widget tree on the current screen"
> "What does this app look like?"

Claude invokes the InkPal MCP tools automatically.

### Manual MCP config (if you can't run `inkpal start`)

If you want to write the MCP config by hand (e.g. on a machine without
Flutter installed), add this to `~/.claude.json`:
...
### About slash commands like `/inkpal:audit`

The MCP server gives Claude the tools (`inkpal_audit_ui`,
`inkpal_lookup_error`, etc.) — Claude calls them via natural language
("audit my UI" routes to `inkpal_audit_ui`).

Polished named slash commands (`/inkpal:audit`, `/inkpal:build`,
`/inkpal:debug`) ship via a separate Claude Code plugin that is in
private beta as of this release. They are convenience, not required —
everything works today through natural-language tool calls. The plugin
will be announced on the Claude Code marketplace; subscribe at
[inkpal.ai](https://inkpal.ai) for the update.
