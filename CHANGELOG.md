...
## 1.0.1 — First signed release (provenance attestation)

First release published through the public mirror repo
[`InkPalAI/inkpal_npm`](https://github.com/InkPalAI/inkpal_npm)
with Sigstore provenance attestation via GitHub Actions OIDC.

Identical surface to 1.0.0 (no functional changes). The `1.0.0`
version slot was tombstoned by a prior unpublish — npm's policy
forbids reusing burned version numbers, so the first lockdown
release ships as `1.0.1`.
